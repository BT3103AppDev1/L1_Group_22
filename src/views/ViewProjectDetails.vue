  ...
  <div class="detail-page">
    ...
    <div class="detail-body" v-else-if="project">
      ...
      <section class="card header-card">
        ...
        <!-- Send Proposal Button - for contractor only -->
      ...
      <!-- Project Inspiration Photos - by homeowner -->
    ...
    onAuthStateChanged(auth, async (user) => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          // Fetch homeowner name from users collection
          if (project.value.homeownerId) {
            const ownerSnap = await getDoc(doc(db, 'users', project.value.homeownerId))
            if (ownerSnap.exists()) {
              const u = ownerSnap.data()
              project.value.homeownerName = u.fullName || u.username || 'Homeowner'
            }
          }
        }
...
<style scoped>
  * {
  ...
  .badge-urgent   { 
    background: #ffe2e2; color: #cc3d3d; 
  }
  .badge-medium   { 
    background: #fff0bf; color: #9b7900; 
  }
  .badge-flexible { 
    background: #ebf6e9; color: #408c48; 
  }
